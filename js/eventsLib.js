...
query='insert into events_type(uuid, event_uuid, type_uuid) values($uuid, $event_uuid, (select uuid from type where categ="invntSrv" and name=$type))';
obj={$uuid:evntTyp_uuid, $event_uuid:evnt_uuid};
  try{
  sqlObj.runQuery(query,obj);
  }
  catch(e){
  console.log('Unable to add entry to "events" table. query: '+query+', binds: '+JSON.stringify(obj));
  console.log(e);
  return null;
  }  
...
let evntIvntSrv_uuid=createUUID();
/* for loop of invntSrvs,
query='insert into events_type(uuid, event_uuid, type_uuid) values($uuid, $event_uuid, (select uuid from type where categ="invntSrv" and name=$type))';
obj={$uuid:evntTyp_uuid, $event_uuid:evnt_uuid};
  try{
  sqlObj.runQuery(query,obj);
